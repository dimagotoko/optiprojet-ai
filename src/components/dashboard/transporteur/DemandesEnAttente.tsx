...
function DemandeCard({ booking, trip }: { booking: Booking; trip: Trip }) {
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      await batch.commit();
      toast({ title: 'Demande acceptée', description: `${traveler?.name ?? 'Voyageur'} a été confirmé(e).` });
    } catch {
      toast({ variant: 'destructive', title: 'Erreur', description: "Impossible d'accepter la demande." });
    } finally {
      setLoading(null);
    }
  };
  ...
  const handleRefuse = async () => {
    ...
    setLoading('refuse');
    try {
      await updateDoc(doc(firestore, 'trips', trip.id, 'bookings', booking.id), {
        status: 'rejected',
      });
      toast({ title: 'Demande refusée' });
    } catch {
      toast({ variant: 'destructive', title: 'Erreur', description: 'Impossible de refuser la demande.' });
    } finally {
      setLoading(null);
    }
  };
    ...
    <div className="rounded-xl border bg-card p-4 flex items-center justify-between gap-4">
      <div className="flex items-center gap-3 min-w-0">
        <Avatar className="h-10 w-10 shrink-0">
          ...
          <AvatarFallback>
            {travelerLoading ? '?' : traveler?.name ? getInitials(traveler.name) : '?'}
          </AvatarFallback>
        </Avatar>
...
function TripPendingBookings({ trip, userId, onCountChange }: TripPendingProps) {
  const firestore = useFirestore();
...
export function DemandesEnAttente({ userId, trips }: DemandesEnAttenteProps) {
  ...
  const handleCountChange = React.useCallback((tripId: string, count: number) => {
    setCounts(prev => {
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        ...
        {allLoaded && totalPending === 0 && (
          <p className="text-sm text-muted-foreground py-1">Aucune demande en attente.</p>
        )}
      </div>
    </div>
  );
