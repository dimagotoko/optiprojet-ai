...
import { Camera, CheckCircle, Trash2, Upload } from "lucide-react";
...
/**
 * Version pré-authentification de AvatarUpload : compresse et prévisualise
 * l'image localement sans l'envoyer à Firebase Storage (aucun uid n'existe
 * encore avant la création du compte). Le blob final est remonté au parent
 * via onChange pour être uploadé une fois le compte créé.
 */
export function AvatarPicker({ displayName, onChange }: AvatarPickerProps) {
  ...
  const processFile = React.useCallback(
    async (file: File) => {
      ...
      try {
        ...
        console.error("[AvatarPicker] compression error:", err);
        setErrorMessage(
